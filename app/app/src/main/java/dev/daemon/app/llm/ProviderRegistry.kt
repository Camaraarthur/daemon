...
class ProviderRegistry(context: Context) {
    ...
    private fun buildProviders(): List<LlmProvider> = listOf(
        EchoProvider(),
        // Local — Gemma 4 E2B via MediaPipe LlmInference, fully on-device once
        // the user downloads the model (~1.87 GB, one-time).
        GemmaProvider(appContext),
        // Local — Gemini Nano via Google's AICore (depends on rollout/enrollment).
    ...
    /** Convenience accessor for the GemmaProvider (used by Settings download UI). */
    fun gemmaProvider(): GemmaProvider =
        providers.first { it.id == "gemma-4-e2b" } as GemmaProvider
