...
data class GenerationResult(
    ...
    /**
     * Honest label for *where the call went*:
     *   "on-device"               → nothing left the phone (Echo, Gemini Nano)
     *   "daemon relay"            → phone → relay.daemon.page → OpenRouter → provider
     *   "your openrouter key"     → BYOK OpenRouter, direct phone → openrouter.ai
     *   "your anthropic key"      → BYOK Anthropic, direct phone → api.anthropic.com
     *   "your mistral key"        → BYOK Mistral, direct phone → api.mistral.ai
     * Never reuse "on-device" for relay/BYOK calls — they DID egress.
     */
    val via: String = "",
)
